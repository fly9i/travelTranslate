...
class VisionTranslateService:
    ...
    def _build_prompt(
        ...
        return (
            ...
            "   ⚠️ 必须穷举真实条目，不要主观筛选：\n"
            "     - menu 场景：每一道菜 / 酒水 / 套餐都必须输出一项，即使价格残缺。\n"
            "     - 其它场景：所有具有实际信息的段落都要输出。\n"
            "   ⛔ 但以下内容必须跳过（不要输出为 item）：\n"
            "     · 店名 / 店招 / Logo / 大横幅标语（如「반지하포차」「某某食堂」「某某餐厅」等）；\n"
            "     · 章节标题 / 分组表头（如「메인 메뉴」「안주류」「주류」「치킨 메뉴」「식사류」「메뉴」；\n"
            "       中文「主食 / 小菜 / 酒水 / 饮料 / 甜点」；英文「Main Menu / Drinks / Appetizers / Desserts」；\n"
            "       日文「お食事 / お飲み物 / おつまみ」等任何仅起分类作用、不是具体商品的标题）；\n"
            "     · 地址 / 电话 / 营业时间 / 税号 / 二维码旁说明 等非商品说明文字；\n"
            "     · 装饰花体字、水印、页码碎片、明显乱码。\n"
            "   🎯 快速判断：一条文字如果没有「价格 / 数量 / 规格 / 描述」与之对应，且从内容看是分类 / 品牌 / 联系方式，"
            "就跳过。\n"
            "3. 合并 / 拆分规则 —— 必须结合 bbox 空间关系判断，不能只看文字内容：\n"
            "   ✅ 合并（同一 item）：当且仅当块 A 与块 B 同时满足：\n"
            "      · 垂直距离 ≤ 1.2 × max(h_A, h_B)（紧邻）；\n"
            "      · 水平上有明显重叠（同一栏/同一行），或下一行是上一行的缩进延续；\n"
            "      · 角色互补（标题 ↔ 描述 ↔ 价格 ↔ 备注），不是两个并列条目。\n"
            "   ❌ 必须拆分：\n"
            "      · 垂直距离 > 1.2 × 行高 —— 视为不同 item。\n"
            "      · 同一栏中两个都像菜名 / 都像标题的平行条目。\n"
            "      · 左右两列（x 差距 > 0.3 且无重叠）通常是独立项目；仅当确认是"
            "「同一行的名 + 价」才合并。\n"
            "   🧭 空间紧邻硬约束：同一 item 内所有 ocr_indices 的 union bbox 的高度必须 ≤ 0.15，"
            "宽度必须 ≤ 0.6（整图归一化）。违反此约束 = LLM 搞错了，必须拆。\n"
            "   💡 拿不准时，宁可拆细也不要合并 —— 粗颗粒是严重错误。\n"
